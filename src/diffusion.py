...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        super().__init__()
        # assert not (type(self) == GaussianDiffusion and model.in_channels != model.out_channels)
    ...
    # @torch.no_grad()
    # def sample(self, target, batch_size = 16):
    #     image_size, channels = self.image_size, self.channels
    #     sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
    #     return sample_fn((batch_size, channels, image_size, image_size), target)
    ...
    def loss_calculation(self,model_out, target, p_thick, t, reduction = 'none'):
        ...
        # return 0.8*noise_loss.mean() + 0.2*p_thick_loss.mean()
