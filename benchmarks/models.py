...
# @Last Modified by:   Jiawen Li
# @Last Modified time: 2024-07-18 18:10:16
...
class SimulatorNet_new_fc(nn.Module):
    def __init__(self, spec_dim=400, d=32):
        super().__init__()
    ...
    def forward(self, shape_in, p_thick):
        shape = self.conv_block_shape(shape_in)
        p_thick = p_thick.view(-1, 4, 1, 1)
        gap = self.conv_block_gap(p_thick)
...
class cVAE_GSNN(nn.Module):
    ...
    def __init__(self, img_size=64, gap_dim=4, spec_dim=400, latent_dim=100, d=16, thickness = [4,1,1], k_size = 3, k_pad = 1):
        ...
        self.final_gap = nn.Sequential(
            nn.Linear(img_size*img_size, gap_dim),
            nn.Tanh()
        )
...
class Generator(nn.Module):
    ...
    def __init__(self, img_size=64, gap_dim=4, spec_dim=58, noise_dim=100, d=16, k_size = 3, k_pad = 1):
        ...
        self.net_fc = nn.Sequential(
            nn.Linear(self.img_size*self.img_size, gap_dim),
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(self, img_size=64, gap_dim=4, spec_dim=400, noise_dim=100, d=16, thickness = [4,1,1], k_size = 3, k_pad = 1):
...
class cGAN(nn.Module):
    ...
    def forward(self, spec, noise):
        ...
        img_fake, gap_fake = self.Generator(spec, noise)
